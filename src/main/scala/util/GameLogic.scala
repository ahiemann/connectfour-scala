...
object GameLogic:
  ...
  def numberOfSuccessivelySymbols(list: List[Boolean], successivelyCount: Int = 0, maxCount: Int = 0): Int =
    ...
    (currentSymbol, tail.headOption) match
      case (Some(current), Some(next)) =>
        if current then // If current symbol is from selected player (true)
  ...
  def getInitialMatchField: MatchfieldModel[PlayerModel] = new MatchfieldModel[PlayerModel](RealPlayer("NoPlayer", '-'))
  ...
  def getEmptyColumns(matchfieldModel: MatchfieldModel[PlayerModel]): List[Int] =
    ...
    val emptyColumns = step.zipWithIndex.map {
        case (element, index) =>
